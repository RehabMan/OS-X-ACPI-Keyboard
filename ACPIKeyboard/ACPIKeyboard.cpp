...
// =============================================================================
// ACPIKeyboard Class Implementation
//
...
OSDefineMetaClassAndStructors(ACPIKeyboard, IOHIKeyboard);

UInt32 ACPIKeyboard::deviceType()
{
...
UInt32 ACPIKeyboard::interfaceID() { return NX_EVS_DEVICE_INTERFACE_ADB; }

UInt32 ACPIKeyboard::maxKeyCodes() { return NX_NUMKEYCODES; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

bool ACPIKeyboard::init(OSDictionary * dict)
{
    //
...
bool ACPIKeyboard::start(IOService * provider)
{
    DEBUG_LOG("ACPIKeyboard::start entered...\n");
...
IOReturn ACPIKeyboard::message(UInt32 type, IOService* provider, void* argument)
{
#if 0
#ifdef DEBUG
    if (argument)
        IOLog("ACPIKeyboard::message: type=%x, provider=%p, argument=%p, argument=0x%04x, cmp=0x%x\n", type, provider, argument, *static_cast<UInt32*>(argument), kIOACPIMessageDeviceNotification);
    else
        IOLog("ACPIKeyboard::message: type=%x, provider=%p", type, provider);
#endif
#endif
...
const unsigned char * ACPIKeyboard::defaultKeymapOfLength(UInt32 * length)
{
    //
